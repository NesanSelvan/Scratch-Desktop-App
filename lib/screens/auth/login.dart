...
class _LoginScreenState extends State<LoginScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      sl<UploadFileCubit>().getDBFileAndUpdateDB();
    });
  }
  ...
  Widget build(BuildContext context) {
    return GetBuilder<LoginController>(
      builder: (controller) {
        return Scaffold(
          key: _scaffoldKey,
          body: BlocListener<UploadFileCubit, UploadFileState>(
            listener: (context, state) {
              if (state is GetFileLoaded) {
                UserResponseUtility.showSuccess(
                  context,
                  "File sync up from server successfully",
                );
              } else if (state is UploadFileError) {
                UserResponseUtility.showError(
                  context,
                  "File sync up from server failed",
                );
              }
            },
            child: Stack(
